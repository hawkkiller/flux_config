...
func (r *Resources) CreateLoadBalancerServices(ctx *pulumi.Context) error {
	for i := range r.ControlPlanes {
		// loadbalance kubectl traffic
		_, err := hcloud.NewLoadBalancerService(ctx, fmt.Sprintf("control-plane-%d-kubectl", i), &hcloud.LoadBalancerServiceArgs{
		...
		_, err = hcloud.NewLoadBalancerService(ctx, fmt.Sprintf("control-plane-%d-talosctl", i), &hcloud.LoadBalancerServiceArgs{
			LoadBalancerId:  r.LoadBalancer.ID(),
			Protocol:        pulumi.String("tcp"),
